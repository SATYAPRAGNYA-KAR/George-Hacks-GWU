...
import { getCountyFPIDetail } from "@/data/baseline";
...
export const CountyDetailPanel = ({ fips }: { fips: string }) => {
  ...
  // Try seeded county first; fall back to baseline detail for synthetic counties
  const seededCounty = getCountyByFips(fips);

  // Derive stateAbbr from FIPS prefix for synthetic lookup
  const stateAbbr = seededCounty?.stateAbbr
    ?? US_STATES.find((s) => fips.startsWith(s.fips))?.abbr
    ?? "";

  // Get FPI detail — works for both seeded and synthetic counties
  const detail = getCountyFPIDetail(fips, stateAbbr);

  // Build a county-like object for display
  const county = seededCounty ?? {
    fips,
    stateAbbr,
    name: detail.name !== `County ${fips}` ? detail.name : `County ${fips}`,
  ...
  const drivers = (seededCounty as any)?.drivers ?? [];

  const totalStock    = orgs.reduce((s, o) => s + o.foodStockLbs, 0);
  const totalVouchers = orgs.reduce((s, o) => s + o.voucherCapacityUsd, 0);

  // Use detail.components which works for synthetic counties too
  const components = [
    { label: "Shock exposure",    value: detail.components.shockExposure,    weight: weights.shockExposure,    icon: AlertTriangle },
    { label: "Vulnerability",     value: detail.components.vulnerability,     weight: weights.vulnerability,    icon: Users },
    { label: "Supply capacity",   value: detail.components.supplyCapacity,   weight: weights.supplyCapacity,   icon: Boxes },
    { label: "Response readiness",value: detail.components.responseReadiness,weight: weights.responseReadiness,icon: Truck },
  ];
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-wider text-muted-foreground">
              {county.stateAbbr} · {county.fips}
            </p>
            <CardTitle className="mt-0.5 text-2xl">{county.name} County</CardTitle>
            <p className="mt-1 text-xs text-muted-foreground">
              Population {(county.population ?? 0).toLocaleString()} · Updated {formatDistanceToNow(new Date(), { addSuffix: true })}
              {!(seededCounty as any)?.seedRich && (
                <span className="ml-2 rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">baseline estimate</span>
              )}
            </p>
          </div>
      ...
      <CardContent className="space-y-6 p-5">
              ...
              <div key={c.label} className="rounded-lg border bg-background/60 p-3">
                ...
                <div className="mt-1.5 flex items-baseline gap-1.5">
                  <span className="text-xl font-bold tabular-nums"
                    style={{ color: TRIGGER_META[triggerForScore(c.value)].color }}>
                    {Math.round(c.value)}
                  </span>
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
                  <div className="h-full rounded-full"
                    style={{ width: `${c.value}%`, backgroundColor: TRIGGER_META[triggerForScore(c.value)].color }} />
                </div>
              </div>
            ))}
          </div>
              ...
              {drivers.map((d: any, i: number) => (
                <div key={i} className="flex items-center justify-between rounded-md border bg-background/60 px-3 py-1.5 text-xs">
                  ...
                  <span className="font-mono tabular-nums"
                    style={{ color: TRIGGER_META[triggerForScore(d.value)].color }}>{d.value}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <section>
          ...
          <div className="space-y-2">
            {recs.length === 0
              ? <p className="text-xs text-muted-foreground">No actions recommended.</p>
              : recs.map((r) => (
        ...
        <section className="grid grid-cols-2 gap-3">
          <div className="rounded-lg border bg-background/60 p-3">
            <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
              <Activity className="h-3 w-3" />Open requests
            </div>
